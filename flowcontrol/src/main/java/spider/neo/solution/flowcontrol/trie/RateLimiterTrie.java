package spider.neo.solution.flowcontrol.trie;

import java.util.HashMap;
import java.util.Map;

class RateLimiterNode{
    Map<String, RateLimiterNode> children = new HashMap<>();
    boolean isEndOfPattern;
    boolean hasWildCard;
}

public class RateLimiterTrie {
    private final RateLimiterNode root = new RateLimiterNode();
    ...
    public void insert(String pattern) {
        ...
        RateLimiterNode node = root;
        for (String part : parts) {
            if (part.equals("*")) {
                node.hasWildCard = true;
                return;
            }
            node.children.putIfAbsent(part, new RateLimiterNode());
    ...
    private String searchHelper(RateLimiterNode node, String[] parts, int index, StringBuilder matched, String lastValidMatch) {
    ...
    private boolean deleteHelper(RateLimiterNode node, String[] parts, int index) {
        if (index == parts.length){
...
//    }
//
//    private void printTrieHelper(RateLimiterNode node, StringBuilder path, int level) {
