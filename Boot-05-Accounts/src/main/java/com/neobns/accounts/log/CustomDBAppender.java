...
public class CustomDBAppender extends DBAppender {
    ...
    private boolean saveLoggingEvent(ILoggingEvent event, PreparedStatement stmt) {
        try {
            ...
            // If args is not null and contains elements, iterate through the first 4 arguments
            if (args != null) {
                for (int i = 0; i < 4; i++) {
                ...
                // If args is null, set the arguments to null
                for (int i = 0; i < 4; i++) {
                    stmt.setString(7 + i, null);
                }
            }

            // Handle MDC values and caller data prioritization
            ...
            if (result == 0) {
                addError("No rows inserted into logging_event table.");
            }
            return true;
